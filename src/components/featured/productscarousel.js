...
function ProductCarousel() {
  ...
  return (
    <div className='relative flex flex-row w-screen h-full justify-center items-center '>
      ...
      <section data-id="B13gDe"  className='flex flex-row overflow-x-scroll snap-x snap-mandatory scroll-smooth scroll-item no-scrollbar h-full w-screen gap-6 before:shrink-0 before:w-[5vw] after:shrink-0 after:w-[5vw]'>{
        data.length > 0 ? data.slice(0, 15).map((item, index) => (
          <a href={`/products/item/${item.slug}`} key={index} className=" w-[200px] h-[250px] md:w-[300px] md:h-[350px] flex flex-col snap-center items-center pt-[1rem] shrink-0 rounded-[10px] gap-2 bg-[#DCBA62] transform transition-all duration-200 hover:bg-[#000] group ">
